    ...
    const addConstraint = (packageData) => {
        ...
        if (packageData.enforce_version) {
            if (!packageJsonData.peerDependencies) {
                logger.warning('Could not find peerDependencies key on @symfony/webpack-encore package');
            ...
            // In the future, we will leverage peer dependencies checks to the package manager.
